...
######clean up that data######
##this data set is an example of RAMALT samples across 3 runs##

df1<-read.xlsx("07072021_ramalt1.xlsx", 1, startRow=2)
df2<-read.xlsx("07092021_ramalt2.xlsx", 1, startRow=2)
df3<-read.xlsx("07132021_ramalt3.xlsx", 1, startRow=2)
meta<-data.frame(read.csv(file="RAMALT_bates_hole_meta.csv"))
...
df1$RunDate<-"07072021"
df2$RunDate<-"07092021"
df3$RunDate<-"07132021"
...
IHC_status<-c(rep("POS",12), rep("ISF",12))
animal_ID<-c(rep("21-40",3), rep("21-37",3), rep("21-43",3), rep("21-05",3),
     rep("21-20",3),rep("21-32",3), rep("21-07",3), rep("21-33",3))
avg_tt<-c(10.6607275,7.7289775, 8.700375,
          14.4631525,12.1131275, 8.572075,
          5.652435, 6.01539, 7.8769725,
          7.97123, 5.05319, 7.829885,
          13.1406225, 15.442225, 9.1144725,
          16.2112025, 20.49113,NA,
          8.2213725, 10.7180575, 8.2454,
          16.8535725,11.2349325,18.3529975)
follicles_pos<-c(rep(7,3), rep(8,3), rep(9,3), rep(6,3), rep(0,12))
...
dd$animal_ID<-factor(dd$animal_ID, levels=c("21-07","21-20","21-33","21-32",
                                            "21-43","21-05","21-40","21-37"))
