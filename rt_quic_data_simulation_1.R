...
colnames(all_samples_final)<-c("wells","names","type", "concentration",paste(c(1:100)))
colnames(pos_ctrls_names)<-c("wells","names","type", "concentration",paste(c(1:100)))

fake_data<-rbind(pos_ctrls_names, all_samples_final)
df<-fake_data

df_melt<-reshape2::melt(df, id.vars=c("wells", "names","type", "concentration"))
df_melt$variable<-as.numeric(df_melt$variable)
df_melt$value<-as.numeric(df_melt$value)

p1<-ggplot(aes(x=variable, y=value, color=as.factor(type)), data=df_melt)+
  geom_point()+geom_line(aes(group=as.factor(wells)))
p1

#write.csv(df, file="RTQ_simulated_data.csv")
